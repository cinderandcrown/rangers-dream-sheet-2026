...
function normalizeTabState(tabKey, tabState) {
  ...
  const stack = Array.isArray(tabState?.stack) && tabState.stack.length
    ? tabState.stack.filter(Boolean)
    : [basePath];
...
export function NavigationProvider({ children }) {
  ...
  const pop = (fallbackTo = TAB_CONFIG[activeTab].path) => {
    ...
    const fallbackPath = stack[currentIndex > 0 ? currentIndex - 1 : stack.length - 2] || fallbackTo;
...
export function useTabNavigation() {
  ...
  if (!context) {
    throw new Error("useTabNavigation must be used inside NavigationProvider");
  }
